...
public class SqlUtil {
    ...
    public static void save(String ImagePath, String Subject, String SchoolYear, String Semester,int difficulty,int reviews,String reviewOf,String subjectId){
        HomeWorkBean bean = new HomeWorkBean( ImagePath,  Subject, SchoolYear, Semester, difficulty , reviews,new Date().getTime(),reviewOf,subjectId);
        getDao().insert(bean);
    }

    public static void saveAll(HomeWorkBean[] homeWorkBeans){
        //if ()
        getDao().insertOrReplaceInTx(homeWorkBeans);
    ...
    public static  List<HomeWorkBean> queryReviewBatch(String subject,String reviewBatch){
        QueryBuilder<HomeWorkBean> builder = getDao().queryBuilder();
        if (!subject.equals("全部") && !reviewBatch.equals("全部")){
            return builder.where(HomeWorkBeanDao.Properties.Subject.eq(subject),HomeWorkBeanDao.Properties.ReviewOf.eq(reviewBatch)).list();
        }
        return  new ArrayList<>();
